...
const UpdateBook = () => {
  ...
  const handleUpdate = async (data) => {
    ...
    try {
      const response=await axios.patch(`http://localhost:8080/books/${id}`, bookData);
      console.log("backend Response:", response.data);
      toast.success("Book updated successfully!");
      navigate("/books/allBooks", { state: { bookUpdated: true } });
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      ...
      <Form {...form}>
        <form onSubmit={handleSubmit(handleUpdate)} className='space-y-6'>
          ...
          <Button type="submit" variant="default" className="w-full bg-blue-600 hover:bg-blue-700 transition-colors duration-300">Update Book</Button>
        </form>
      </Form>
      </div>
    </div>
